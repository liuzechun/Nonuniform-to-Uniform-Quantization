...
parser.add_argument('--data', metavar='DIR', help='teacher model')
...
def main():
    ...
    for pname, p in model_student.named_parameters():
        ...
        if 'quan1.a' in pname or 'quan2.a' in pname or 'quan3.a' in pname or 'scale' in pname or 'start' in pname:
    ...
    checkpoint_tar = os.path.join(args.save, 'real_mbv2.pth.tar')
    if not os.path.exists(checkpoint_tar):
        if not os.path.exists(args.save):
            os.makedirs(args.save)
        import gdown # pip install gdown
        gdown.download(id='1Rbf8jQuisuLdKB9Cfth962I2RBCwm-Gu', output=checkpoint_tar, quiet=False)
